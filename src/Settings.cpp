...
void Settings::load() {
  ...
  if (cfgFile.open(QIODevice::ReadOnly)) {
    ...
    if (!m_settings.contains(OPTION_WALLET_FILE)) {
      m_addressBookFile = getDataDir().absoluteFilePath(QCoreApplication::applicationName() + ".contacts");
    } else {
      m_addressBookFile = m_settings.value(OPTION_WALLET_FILE).toString();
      m_addressBookFile.replace(m_addressBookFile.lastIndexOf(".wallet"), 7, ".contacts");
    }
  } else {
    m_addressBookFile = getDataDir().absoluteFilePath(QCoreApplication::applicationName() + ".contacts");
  }
  ...
  defaultPoolList << "pool.xdrop.io:3333" << "pool.xdrop.io:4444" << "pool.xdrop.io:5555";
...
#ifdef Q_OS_WIN
  if (!m_settings.contains("messageNotifications")) {
    m_settings.insert("messageNotifications", true);
    saveSettings();
  }

  if (!m_settings.contains("paymentNotifications")) {
    m_settings.insert("paymentNotifications", false);
    saveSettings();
  }
#endif

  if (!m_settings.contains("readMessages")) {
    m_settings.insert("readMessages", 0);
    saveSettings();
  }
...
bool Settings::isMessageNotificationsEnabled() const {
  return m_settings.contains("messageNotifications") ? m_settings.value("messageNotifications").toBool() : false;
}
bool Settings::isPaymentNotificationsEnabled() const {
  return m_settings.contains("paymentNotifications") ? m_settings.value("paymentNotifications").toBool() : false;
}
#endif
...
void Settings::setReadMessages(const quint16& _readMessages) {
  m_settings.insert("readMessages", _readMessages);
  saveSettings();  
}

quint16 Settings::getReadMessages() {
  return m_settings.value("readMessages").toInt();
}
...
void Settings::setMessageNotificationsEnabled(bool _enable) {
  if (isMessageNotificationsEnabled() != _enable) {
    m_settings.insert("messageNotifications", _enable);
    saveSettings();
  }
}

void Settings::setPaymentNotificationsEnabled(bool _enable) {
  if (isPaymentNotificationsEnabled() != _enable) {
    m_settings.insert("paymentNotifications", _enable);
    saveSettings();
  }
}
#endif
