...
WalletAdapter::WalletAdapter() : QObject(), m_wallet(nullptr), m_mutex(), m_isBackupInProgress(false),
  ...
  connect(this, &WalletAdapter::walletSendMessageCompletedSignal, this, &WalletAdapter::onWalletSendTransactionCompleted, Qt::QueuedConnection);
...
bool WalletAdapter::changePassword(const QString& _oldPassword, const QString& _newPassword) {
  ...
  Q_EMIT changeWalletPasswordSignal();
...
void WalletAdapter::updateBlockStatusText() {
  ...
  const QString miningState = MiningFrame::getMiningStatus();
  ...
  const QString warningString = blockTime.msecsTo(currentTime) < LAST_BLOCK_INFO_WARNING_INTERVAL ? 
    QString("Last block found %1 minutes ago").arg(blockAge % MSECS_IN_HOUR / MSECS_IN_MINUTE) :
  ...
  Q_EMIT walletStateChangedSignal(
    QString(tr("Wallet synchronized | Block height: %1 | %2 | %3"))
...
void WalletAdapter::updateBlockStatusTextWithDelay() {
  QTimer::singleShot(500, this, SLOT(updateBlockStatusText()));
}
