...
QString MINING_STATUS = "Not Mining"; 
...
QString MiningFrame::getMiningStatus()  {
...
void MiningFrame::timerEvent(QTimerEvent* _event) {
  if (_event->timerId() == m_hashRateTimerId) {
    ...
    MINING_STATUS = tr("Hashrate: %1 H/s").arg(hashRate);
    m_ui->m_stateLabel->setText(MINING_STATUS);
    return;
  }
...
void MiningFrame::startMining() {
  ...
  MINING_STATUS = tr("Starting to mine...");
  m_ui->m_stateLabel->setText(MINING_STATUS);
