...
void ReceiveFrame::copyAddress() {
  ...
  QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Successfully copied the address to your clipboard"), QtInfoMsg));
}

void ReceiveFrame::copyKey() {
  QApplication::clipboard()->setText(m_ui->m_keyEdit->text());
  QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Successfully copied the key to your clipboard"), QtInfoMsg));
}
